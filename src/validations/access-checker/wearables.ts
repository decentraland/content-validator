...
import { EntityWithEthAddress, validationFailed, OK, Validation } from '../../types'
...
export const wearables: Validation = {
  validate: async ({ externalCalls, theGraphClient, logs }, deployment) => {
    ...
    const getCollectionItems = async (
      ...
      const result = await subgraphUrl.query<WearableCollections>(query, {
        collection,
    ...
    const getMerkleRoot = async ({
      ...
      const result = await externalCalls.subgraphs.L2.thirdPartyRegistry.query<{
    ...
    const verifyMerkleProofedEntity = async (urn: BlockchainCollectionThirdParty): Promise<boolean> => {
      ...
      const { blockNumberAtDeployment, blockNumberFiveMinBeforeDeployment } =
        await theGraphClient.findBlocksForTimestamp(externalCalls.subgraphs.L2.blocks, deployment.entity.timestamp)
