  ...
  validate: async ({ deployment, externalCalls }, logs) => {
    ...
    const verifyHash = async (metadata: ThirdPartyWearable, merkleRoot: string): Promise<boolean> => {
      ...
      if (!MerkleProof.validate(metadata.merkleProof)) {
        console.log('Merkle proof is not valid', { merkleProof: metadata.merkleProof })
        return false
      }
      ...
      if (!MERKLE_PROOF_REQUIRED_KEYS.every((key) => merkleProof.hashingKeys.includes(key))) {
        console.log(`Merkle proof hashing keys don't satisfy the required keys`, {
      ...
      if (metadata.merkleProof.entityHash !== generatedCrcHash) {
        console.log(`The hash provided in the merkleProof doesn't match with the generated by the validator`, {
          generatedCrcHash,
    ...
    const verifyMerkleProofedEntity = async (urn: BlockchainCollectionThirdParty): Promise<boolean> => {
      ...
      const hasPermissionOnBlock = async (blockNumber: number | undefined) => {
        ...
        if (!merkleRoot) {
          console.log(`Merkle proof not found for given block and third party ID`, { blockNumber, thirdPartyId })
          return false
        }
      ...
      if (!validMerkleProofedEntity) {
        console.log(`Merkle Tree based verifications failed:`, {
