import { createStoreValidateFn } from '../../../src/validations/on-chain-access-checker/stores'
import { buildStoreDeployment } from '../../setup/deployments'
import { buildOnChainAccessCheckerComponents, buildExternalCalls } from '../../setup/mock'
...
describe('Access: stores', () => {
  it('When a user store is created by its own address, then it is valid', async () => {
    ...
    const validateFn = createStoreValidateFn(buildOnChainAccessCheckerComponents({ externalCalls }))
  ...
  it('When a profile is created and the pointers does not match the signer, the access check fails', async () => {
    ...
    const pointer = 'urn:decentraland:off-chain:marketplace-stores:' + otherAddress
