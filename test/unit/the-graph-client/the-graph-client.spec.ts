import { buildComponents, buildExternalCalls, fetcherWithWearablesOwnership, mockedQueryGraph } from '../../setup/mock'
...
describe('TheGraphClient', () => {
  ...
  it('When invoking findBlocksForTimestamp, it may happen that no block matches and exception is thrown', async () => {
    ...
    await expect(theGraphClient.findBlocksForTimestamp('blocksSubgraph', 10)).rejects.toThrow('Internal server error')
  })
  ...
  describe('Checks for name ownership', function () {
    it('When no block for current timestamp, it should continue and check the block from 5 minute before', async () => {
      const externalCalls = buildExternalCalls({
        queryGraph: mockedQueryGraph().mockImplementation(async (url, _query, _variables) => {
